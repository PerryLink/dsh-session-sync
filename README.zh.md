...
| 项目 | 状态 |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.5-alpha.1`（GitHub tag，2026-09-09 已核验：完整门控链 + profile 安装冒烟）。npm 依赖线 `0.1.2-rc.1`，peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0`。（2026-09-09 已适配）：会话信封保留 ignorable 字段但仅用于存量日志读取兼容——Session.append 仍无法盖章，门控行为不变。 |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
pnpm run typecheck && pnpm run typecheck:ci        # tsc --checkJs，针对已发布的 0.1.2-rc.1 peers
